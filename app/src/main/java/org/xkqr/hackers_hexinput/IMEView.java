...
public class IMEView extends View {
    ...
    protected void onDraw(Canvas canvas) {
     ...
     //   rect.setTextSize(getResources().getDimensionPixelSize(R.dimen.key_label_size));

//        float sz = (float)Math.min(h / (2.0 * hexrs), w / Math.sqrt(3) / hexqs);

        // create pathshapes for the hexagons?
        for (HexCoordinate coord : _kb.grid()) {
            double x = coord.x();
            double y = coord.y();
            ...
            canvas.drawText(_kb.primary(coord), (float)(_scale*x), (float)(_scale*y), rect);
            //canvas.drawText(coord.q() + "," + coord.r(), (float)(_scale*x), (float)(_scale*y), rect);
